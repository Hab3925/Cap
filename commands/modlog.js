...
module.exports.run = async (client, message, args) => {
    ...
    if (args[0].toLowerCase() == 'disable') { client.logchn.get(message.guild.id) == 'disabled' ? message.channel.send(`I can't remember sending mod logs anywhere...`) : message.channel.send(`Ok, I'll stop sending mod logs to <#${client.logchn.get(message.guild.id)}>`); client.logchn.set(message.guild.id, 'disabled'); return }
    //*Ctrl+C* "JOINK" *Ctrl+V*
    ...
    } else { message.channel.send("That's not a channel in this server, make sure to mention it!") }
}
